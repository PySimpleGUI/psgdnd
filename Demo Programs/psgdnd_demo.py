...
def main():
    ...
    log = [[sg.Text('Event log — every drop is a DropEvent; values[event] is the text or the comma-joined paths', font='_ 10 bold')],
           [sg.Multiline(key=K_LOG, size=(78, 30), font='Courier 9', autoscroll=True, write_only=True, disabled=True)],
    ...
    window[K_STATUS].update(f'{count} drop targets registered', text_color='#7CD992')
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED:
            ...
            if event.key == K_GRAPH:                                             # EVENT_ONLY target: the app decides what a drop means
                ...
                window[K_LOG].print('   (window background — nothing registered under the cursor)', text_color='#FFD479')
        elif event == K_PATH:                                                    # the Input's own event still fires after a drop
            window[K_LOG].print(f'{event}  value changed → {values[K_PATH]}', text_color='#C3F09C')
        else:
